...
int SDCard_Init(void)
{
  ...
  if (ret) {
	...
	// Phone number
	// if (f_gets(line, LINELEN, &file) == NULL || sscanf(line, "PHONENUMBER = %llu", &pnumber) != 1 || pnumber < 100000000 || pnumber > 9999999999) {
	if (f_gets(line, LINELEN, &file) == NULL || sscanf(line, "PHONENUMBER = %llu", &pnumber) != 1 ) {// modified by CSW, original one is the above line.
