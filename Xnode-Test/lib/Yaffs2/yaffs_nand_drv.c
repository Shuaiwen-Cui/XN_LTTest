...
static inline uc8 *dev_to_buffer(struct yaffs_dev *dev)
{
	struct nand_context *ctxt =
		(struct nand_context *)(dev->driver_context);
	return ctxt->buffer;
...
static int NANDfunc_read_cycle(struct nand_chip * this)
{
...
int yaffs_nand_install_drv(struct yaffs_dev *dev, struct nand_chip *chip)
{
	...
	ctxt = sdcalloc(1,sizeof(struct nand_context));
	buffer = sdcalloc(1,chip->spare_bytes_per_page);
	configASSERT(ctxt && buffer);
	if(!buffer || !ctxt)
		goto fail;
	...
	chip->read_cycle = NANDfunc_read_cycle;
/*	chip->set_ale = NANDfunc_set_ale;
	chip->set_cle = NANDfunc_set_cle;
	chip->read_cycle = 
	chip->write_cycle = 
	chip->check_busy = 
	chip->idle_fn = 
