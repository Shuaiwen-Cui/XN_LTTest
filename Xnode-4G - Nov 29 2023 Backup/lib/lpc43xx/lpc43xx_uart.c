 ...
 */
 ...
 // TUFIXDONE: Bring this driver as close to the original as possible - If it works, it works - Don't fix
...
static Status uart_set_divisors(LPC_USARTn_Type *UARTx, uint32_t baudrate, uint32_t clockspeed)
{
	...
	* 0 < mulFracDiv <= 15, 0 <= dividerAddFracDiv <= 15 */
	...
	for (m = 1 ; m <= 15 ;m++)
	{
		for (d = 0 ; d < m ; d++)
		{
		  divisor = ((uint64_t)uClk<<28)*m/(baudrate*(m+d)); // 32- 28 = 4, 2^4 = 16
	...
	//		UARTx->OSR = 32 & 0xf0; // 4000000
	//		UARTx->OSR = 176 & 0xf0; // 1000000
	//		UARTx->OSR = 192 & 0xf0; // 921600
		}
		errorStatus = (Status) !ERROR;	// really? "enumerated type mixed with another" fixed by that casting?
	}
...
{
	CHECK_PARAM(PARAM_UARTx(UARTx));
	//TIM_Waitms(1); // testing shows that this helps reducing losing data at receiving end
	if (((LPC_UART1_Type *)UARTx) == LPC_UART1)
	{
...
uint32_t UART_Send(LPC_USARTn_Type *UARTx, uint8_t *txbuf, uint32_t buflen, TRANSFER_BLOCK_Type flag, bool iswait)
{
	...
	if (flag == BLOCKING) {
		...
		while (bToSend){
			...
			if (iswait) { TIM_Waitms(1);}	// TUFIX: Why is this necessary for --EOF--Pattern-- ? 
		}
	}
	// None blocking mode
	else {
...
uint32_t UART_RS485Send(LPC_USARTn_Type *UARTx, uint8_t *pDatFrm, uint32_t size, uint8_t ParityStick)
{
	...
	if (((LPC_UART1_Type *)UARTx) == LPC_UART1)
	{
		if (ParityStick){
			...
			cnt = UART_Send((LPC_USARTn_Type *)UARTx, pDatFrm, size, BLOCKING, 0);
