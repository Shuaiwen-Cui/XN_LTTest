...
void yaffs_calc_oldest_dirty_seq(struct yaffs_dev *dev)
{
	...
	for (i = dev->internal_start_block; i <= dev->internal_end_block; i++) {
		if (b->block_state == YAFFS_BLOCK_STATE_FULL &&
		    (uc32)(b->pages_in_use - b->soft_del_pages) <
 ...
 * Only for yaffs2.
 */
uc32 yaffs2_find_refresh_block(struct yaffs_dev *dev)
{
	uc32 b;
	uc32 oldest = 0;
	uc32 oldest_seq = 0;
...
int yaffs_calc_checkpt_blocks_required(struct yaffs_dev *dev)
{
	...
	if (!dev->checkpoint_blocks_required && yaffs2_checkpt_required(dev)) {
		...
		n_bytes +=
		    (sizeof(struct yaffs_checkpt_obj) + sizeof(uc32)) *
		    dev->n_obj;
		n_bytes += (dev->tnode_size + sizeof(uc32)) * dev->n_tnodes;
		n_bytes += sizeof(struct yaffs_checkpt_validity);
		n_bytes += sizeof(uc32);	/* checksum */
					     ...
					     struct yaffs_checkpt_validity *v)
{
	...
	v->magic = swap_uc32(v->magic);
	v->version = swap_uc32(v->version);
	v->head = swap_uc32(v->head);
}
				     ...
				     struct yaffs_checkpt_dev *cp)
{
	...
	cp->alloc_page = swap_uc32(cp->alloc_page);
...
static int yaffs2_wr_checkpt_dev(struct yaffs_dev *dev)
{
	struct yaffs_checkpt_dev cp;
	uc32 n_bytes;
	uc32 n_blocks = dev->internal_end_block - dev->internal_start_block + 1;
	int ok;
	uc32 i;
	...
	if (!dev->swap_endian) {
		...
		for (i = 0; i < n_blocks && ok; i++) {
			...
			bu.as_uc32[0] = swap_uc32(bu.as_uc32[0]);
			bu.as_uc32[1] = swap_uc32(bu.as_uc32[1]);
...
static int yaffs2_rd_checkpt_dev(struct yaffs_dev *dev)
{
	...
	if (dev->swap_endian) {
		/* The block info can just be handled as a list of uc32s. */
		uc32 *as_uc32 = (uc32 *) dev->block_info;
		uc32 n_uc32s = n_bytes/sizeof(uc32);
		uc32 i;

		for (i=0; i < n_uc32s; i++)
			as_uc32[i] = swap_uc32(as_uc32[i]);
	}
...
static uc32 yaffs2_checkpt_obj_bit_get(struct yaffs_checkpt_obj *cp,
				      int bit_offset,
				      int bit_width)
{
	uc32 and_mask;
				     ...
				     struct yaffs_checkpt_obj *cp)
{
	...
	uc32 cp_variant_type = yaffs2_checkpt_obj_bit_get(cp, CHECKPOINT_VARIANT_BITS);
...
{
	int i;
	uc32 *as_uc32 = (uc32 *)tn;
	int tnode_size_uc32 = dev->tnode_size / sizeof(uc32);

	if (!dev->swap_endian)
		return;
	/* Swap all the tnode data as uc32s to fix endianisms. */
	for (i = 0; i<tnode_size_uc32; i++)
		as_uc32[i] = swap_uc32(as_uc32[i]);
}
				       ...
				       int chunk_offset)
{
	...
	yaffs_do_endian_uc32(dev, &base_offset);
...
static int yaffs2_rd_checkpt_tnodes(struct yaffs_obj *obj)
{
	...
	yaffs_do_endian_uc32(dev, &base_chunk);
					 ...
					 struct yaffs_checkpt_obj *cp)
{
	if (!dev->swap_endian)
		return;
	cp->struct_type = swap_s32(cp->struct_type);
	cp->obj_id = swap_uc32(cp->obj_id);
	cp->parent_id = swap_uc32(cp->parent_id);
	cp->hdr_chunk = swap_s32(cp->hdr_chunk);
	cp->bit_field = swap_uc32(cp->bit_field);
...
static int yaffs2_wr_checkpt_sum(struct yaffs_dev *dev)
{
	...
	yaffs_do_endian_uc32(dev, &checkpt_sum);
...
static int yaffs2_rd_checkpt_sum(struct yaffs_dev *dev)
{
	...
	if (!ok)
		return 0;
	yaffs_do_endian_uc32(dev, &checkpt_sum1);
		...
		*found_chunks = 1;
		...
		if (in->valid) {
			...
			if ((in->variant_type == YAFFS_OBJECT_TYPE_FILE) &&
				...
				uc32 parent_obj_id = (oh) ?
					(uc32)oh->parent_obj_id :
		...
		if (!in->valid && in->variant_type !=
			...
			yaffs_trace(YAFFS_TRACE_ERROR,
				...
				chunk);
			in = yaffs_retype_obj(in, (enum yaffs_obj_type)(oh ? oh->type : tags.extra_obj_type));
		}
	...
	for (blk = dev->internal_start_block; blk <= dev->internal_end_block;
		...
		if (bi->block_state == YAFFS_BLOCK_STATE_CHECKPOINT) {
			...
			yaffs_trace(YAFFS_TRACE_BAD_BLOCKS,
				"yaffs2_scan_backwards block %d is bad", blk);
