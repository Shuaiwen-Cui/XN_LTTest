...
void ReliableComm_receive(uint16_t addr, uint16_t *addrs, uint8_t addrlen, uint8_t *data, uint32_t len, int success)
{
	...
	if (cmds[id].remote) {
		...
		if (cmds[id].cmd != NULL && cmds[id].inprogress) {
			PRINTF("RemoteCommand[%u] execute command: ERROR: ReliableComm_receive execution already in progress\r\n", id);
			return;
		} else if (m->hdr.resp) {
			return;
		}
		...
		if (cmds[id].cmd == NULL || !cmds[id].inprogress || !m->hdr.resp) {
			PRINTF("RemoteCommand[%u] return data: ERROR: ReliableComm_receive execution already in progress\r\n", id);
			return;
		}
