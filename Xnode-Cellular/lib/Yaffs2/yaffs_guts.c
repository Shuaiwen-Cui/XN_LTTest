...
static int yaffs_wr_data_obj(struct yaffs_obj *in, int inode_chunk,
			     const uc8 *buffer, int n_bytes, int use_reserve);
				...
				int *chunk_out, uc32 *offset_out)
{
	...
	chunk = (uc32) (addr >> dev->chunk_shift);

	if (dev->chunk_div == 1) {
		/* easy power of 2 case */
		offset = (uc32) (addr & dev->chunk_mask);
	} else {
		/* Non power-of-2 case */
...
static inline uc32 calc_shifts_ceiling(uc32 x)
{
	int extra_bits;
	int shifts;
...
static inline uc32 calc_shifts(uc32 x)
{
	uc32 shifts;
...
{
	int retval = YAFFS_OK;
	uc8 *data = yaffs_get_temp_buffer(dev);
				      ...
				      struct yaffs_ext_tags *tags)
{
	...
	uc8 *buffer = yaffs_get_temp_buffer(dev);
	int result;
...
static uc16 yaffs_calc_name_sum(const YCHAR *name)
{
	uc16 sum = 0;
	uc16 i = 1;
...
static int yaffs_tags_match(const struct yaffs_ext_tags *tags, int obj_id,
			    int chunk_obj)
{
	return (tags->chunk_id == (uc32)chunk_obj &&
		tags->obj_id == (uc32)obj_id &&
		!tags->is_deleted) ? 1 : 0;
...
static struct yaffs_tnode *yaffs_prune_worker(struct yaffs_dev *dev,
					      struct yaffs_tnode *tn, uc32 level,
					      int del0)
{
	int i;
	int has_data;
	...
	if (level > 0) {
		...
		int tnode_size_uc32 = dev->tnode_size / sizeof(uc32);
		uc32 *map = (uc32 *) tn;

		for (i = 0; !has_data && i < tnode_size_uc32; i++) {
			if (map[i])
				has_data++;
		}
	}
...
static struct yaffs_obj *yaffs_create_obj(enum yaffs_obj_type type,
					  ...
					  const YCHAR *alias_str, uc32 rdev)
{
	struct yaffs_obj *in;
	YCHAR *str = NULL;
					...
					int old_chunk, uc8 *buffer)
{
	...
	if (object &&
		...
		if (tags.chunk_id == 0) {
			...
			new_chunk =
			    yaffs_write_new_chunk(dev, (uc8 *) oh, &tags, 1);
		} else {
			new_chunk =
...
static int yaffs_rd_data_obj(struct yaffs_obj *in, int inode_chunk, uc8 * buffer)
{
				  ...
				  void *value, int size)
{
	...
	result =
	    yaffs_rd_chunk_tags_nand(dev, obj->hdr_chunk, (uc8 *) buffer, &tags);
	...
	yaffs_release_temp_buffer(dev, (uc8 *) buffer);
	return retval;
}
...
int yaffs_file_rd(struct yaffs_obj *in, uc8 * buffer, loff_t offset, int n_bytes)
{
	int chunk;
	uc32 start;
	int n_copy;
	...
	while (n > 0 && chunk_written >= 0) {
		...
		if (n_copy != (int)dev->data_bytes_per_chunk ||
			...
			if (dev->param.n_caches > 0) {
				...
				uc8 *local_buffer = yaffs_get_temp_buffer(dev);
...
int yaffs_wr_file(struct yaffs_obj *in, const uc8 *buffer, loff_t offset,
		  int n_bytes, int write_through)
{
...
int yaffs_get_obj_name(struct yaffs_obj *obj, YCHAR *name, int buffer_size)
{
	...
	if (obj->obj_id == YAFFS_OBJECTID_LOSTNFOUND) {
		...
		int result;
		uc8 *buffer = yaffs_get_temp_buffer(obj->my_dev);
...
int yaffs_guts_initialise(struct yaffs_dev *dev)
{
	...
	if (!init_failed) {
		dev->gc_cleanup_list =
		    kmalloc(dev->param.chunks_per_block * sizeof(uc32),
					GFP_NOFS);
			...
			int do_endian)
{
	...
	if (do_endian) {
		yaffs_do_endian_uc32(dev, &oh->file_size_low);
		yaffs_do_endian_uc32(dev, &oh->file_size_high);
	}
}
			...
			int do_endian)
{
	...
	if (sizeof(loff_t) >= 8 && ~(oh->file_size_high)) {
		uc32 low = oh->file_size_low;
		uc32 high = oh->file_size_high;

		if (do_endian) {
			yaffs_do_endian_uc32 (dev, &low);
			yaffs_do_endian_uc32 (dev, &high);
		}
