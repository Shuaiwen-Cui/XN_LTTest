...
struct yaffsfs_FileDes {
	uc8 isDir:1; 		/* This s a directory */
	uc8 reading:1;
	uc8 writing:1;
	uc8 append:1;
	uc8 shareRead:1;
 ...
 */
struct yaffs_dev *yaffsfs_FindDevice(const YCHAR *path,
					    YCHAR **restOfPath)
{
	struct list_head *cfg;
	const YCHAR *leftOver = (YCHAR*)sdmalloc(YAFFS_MAX_NAME_LENGTH + 1);
	const YCHAR *p = (YCHAR*)sdmalloc(YAFFS_MAX_NAME_LENGTH + 1);
	struct yaffs_dev *retval = NULL;
	struct yaffs_dev *dev = NULL;
	int thisMatchLength;
	int longestMatch = -1;
	int matching;
	configASSERT(leftOver && p);
	/*
					    ...
					    int *notDir, int *loop)
{
	struct yaffs_obj *dir;
	struct yaffs_obj *obj;
	YCHAR *name = (YCHAR*)sdmalloc(YAFFS_MAX_NAME_LENGTH + 1);
	configASSERT(name);
			...
			int oflag, int mode, int sharing)
{
	...
	uc8 shareRead = (sharing & YAFFS_SHARE_READ) ? 1 : 0;
	uc8 shareWrite = (sharing & YAFFS_SHARE_WRITE) ? 1 : 0;
	uc8 sharedReadAllowed;
	uc8 sharedWriteAllowed;
	uc8 alreadyReading;
	uc8 alreadyWriting;
	uc8 readRequested;
	uc8 writeRequested;
	...
	if (handle < 0) {
		...
		if (obj) {
			...
			is_dir = (obj->variant_type ==
					YAFFS_OBJECT_TYPE_DIRECTORY);
		lpc_printf("The file already exists or it might be a directory\n\r"); 
				...
				const YCHAR *path, int isDirectory)
{
	...
	return (result == YAFFS_FAIL) ? YAFFS_FAIL : YAFFS_OK;
}
			...
			const YCHAR *oldPath, const YCHAR *newPath)
{
	...
	YCHAR *oldname = (YCHAR*)sdmalloc(YAFFS_MAX_NAME_LENGTH + 1);
	YCHAR *newname = (YCHAR*)sdmalloc(YAFFS_MAX_NAME_LENGTH + 1);
	int result = YAFFS_FAIL;
	...
	int oldLoop = 0;
	int newLoop = 0;
	YCHAR *alt_newpath = (YCHAR*)sdmalloc(YAFFS_MAX_NAME_LENGTH + 1);
	configASSERT(oldname && newname && alt_newpath);
...
int yaffs_fstat(int fd, struct yaffs_stat *buf)
{
	...
	int retVal = -1;
	obj = (struct yaffs_obj*)sdcalloc(1,sizeof(struct yaffs_obj));
	configASSERT(obj);
...
{
	int result;
	YCHAR *alt_path = (YCHAR*)sdmalloc(YAFFS_MAX_NAME_LENGTH + 1);
	configASSERT(alt_path);
...
{
	struct yaffs_dev *dev = NULL;
	YCHAR *dummy = (YCHAR*)sdmalloc(YAFFS_MAX_NAME_LENGTH + 1);
	configASSERT(dummy);	
				...
				int read_only, int skip_checkpt)
{
	...
	if (dev) {
		if (!dev->is_mounted) {
			...
			if (skip_checkpt) {
				skip = dev->param.skip_checkpt_rd; 
...
int yaffs_inodecount_common(struct yaffs_dev *dev, const YCHAR *path)
{
	...
	if (retVal < 0) yaffsfs_SetError(-EINVAL);
...
int yaffs_dump_dev_reldir(struct yaffs_obj *reldir, const YCHAR *path)
{
	...
	(void) reldir;
#else
	YCHAR *rest = (YCHAR*)sdmalloc(YAFFS_MAX_NAME_LENGTH + 1);
	configASSERT(rest);
