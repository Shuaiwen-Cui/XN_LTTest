...
extern int SD_ISIN;		// assume as is in, if it's not in, it's switched to 0 and never used again (skipped in fchir)
...
)
{
	...
	for (;;) {
		...
		if (di >= _MAX_LFN)				/* Reject too long name */
			return FR_INVALID_NAME;
		#if !_LFN_UNICODE // copied this fragment of code from https://www.avrfreaks.net/sites/default/files/forum_attachments/ff_0.c to overcome the warning: Statement is unreachable
		w &= 0xFF;
