...
static uint8_t rsidx, rsok, rcid = 0xff;
...
static void getDatafromSD(void *parameterss)
{
	...
	if ((sensor_latestselect)&&(max==0)) { NodeReset();}
	if ((!sensor_latestselect)&&(min==315360000ULL)) { NodeReset();}
...
static void Retrieveprocessdata(uint8_t *data, uint32_t len)
{
	...
	lpc_printf("- received %u bytes at address %0xh\r\n", len, (uint32_t)data);
	...
	vcinfo[2*rsidx+1] = (sd->current < 100.f) ? 0.f : sd->current;
	...
	for (i = 0; i < sd->channels[0].sampSize; i++) {
		...
		for (j = 0; j < rschannels; ++j) {
			if (j < 3) {
				...
				k += snprintf(line + k, 120 - k, "\t\t%.6f", sd->channels[j].sampData[i]);
			}
		}
  ...
  xTaskToNotify = xTaskGetCurrentTaskHandle();
	...
	rcid = RC_CMD_DR_RETRIEVE;
		for (rsidx = 0, rsok = 0; rsidx < rsncnt; ++rsidx) {
			...
			xTimerChangePeriod(tim, 60000, portMAX_DELAY); // for 8 channels, 300 sec take longer - need a function for this wait time to optimize it
