...
static const char params_menu[] =
	...
	"- '4'  Change node list\r\n"
	"- '5'  Toggle retrieving data\r\n"
	"- '6'  Toggle printing data\r\n"
	"- '7'  Change calibration parameters\r\n"
	"- '8'  Return\r\n"
	"- Xnode> ";
...
static const char scale_menu[] = 
	"- Enter channel %u scaling factor: ";

static const char offset_menu[] = 
	"- Enter channel %u offset: ";
...
static const char time_date_menu[] =
	"- Enter date (YY-MM-DD): ";

static const char time_time_menu[] =
	"- Enter time (hh:mm:ss): ";
...
void setdatetime(void)
{
	...
	line[i] = '\0';
	lpc_printf("\r\n");
	if (sscanf(line, "%u-%u-%u", &year, &month, &day) != 3) {
		lpc_printf("ERROR: bad input!\r\n");
		return;
	}
	lpc_printf(time_time_menu);
	for (i = 0; i < LINELEN;) {
		ch = GetChar();
		if (!ch) {
			continue;
	...
	line[i] = '\0';
	lpc_printf("\r\n");
	if (sscanf(line, "%u:%u:%u", &hour, &minute, &second) != 3) {
		lpc_printf("ERROR: bad input!\r\n");
		return;
	}
	settime((uint8_t)year, (uint8_t)month, (uint8_t)day, (uint8_t)hour, (uint8_t)minute, (uint8_t)second);
}
...
int RemoteSensing_setparams(void)
{
	...
	if (doreadcfg("RemoteSensing.cfg") != SUCCESS) {
		lpc_printf("ERROR: failed to read configuration!\r\n");
		return FAIL;
	}
	...
	case 4: // node list
		...
		break;
	case 5: // retrieve data toggle
		lpc_printf("- Retrieve data after sensing (y/n): ");
		GetChar();
		do {
			ch = GetChar();
		} while (!ch);
		...
		if (ch == 'y' || ch == 'Y') {
			...
			rsretrievedata = false;
		} else {
			lpc_printf("ERROR: bad input!\r\n");
			return FAIL;
		}
		break;
	case 6: // print data toggle
		lpc_printf("- Print data after retrieving (y/n): ");
		GetChar();
		do {
			ch = GetChar();
		} while (!ch);
		...
		break;
	case 7: // calibration parameters
		for (j = 3; j < 8; ++j) {
			lpc_printf(scale_menu, j + 1);
			for (i = 0; i < LINELEN;) {
				ch = GetChar();
				if (!ch) {
					continue;
			...
			line[i] = '\0';
			lpc_printf("\r\n");
			if (sscanf(line, "%f", &s) != 1) {
				lpc_printf("ERROR: bad input!\r\n");
				return FAIL;
			}
			rscalibinfo[j - 3].scale = s;
			lpc_printf(offset_menu, j + 1);
			for (i = 0; i < LINELEN;) {
				ch = GetChar();
				if (!ch) {
					continue;
			...
			line[i] = '\0';
			lpc_printf("\r\n");
			if (sscanf(line, "%f", &o) != 1) {
				lpc_printf("ERROR: bad input!\r\n");
				return FAIL;
			}
			rscalibinfo[j - 3].offset = o;
		}
		break;
	default:
		return SUCCESS;
	...
	// retrieve data
	snprintf(line, LINELEN, "RETRIEVEDATA = %u\n", rsretrievedata);
	...
	// print data
	snprintf(line, LINELEN, "PRINTDATA = %u\n", rsprintdata);
	if (f_puts(line, &file) == 0) {
		PRINTF("Error writing data\r\n");
		f_close(&file);
		return FAIL;
	...
	// calibration data
	for (j = 3; j < 8; ++j) {
		snprintf(line, LINELEN, "CH%1uSCALE = %f\n", j+1, rscalibinfo[j-3].scale);
		if (f_puts(line, &file) == 0) {
			PRINTF("Error writing data\r\n");
			f_close(&file);
			return FAIL;
		}
		snprintf(line, LINELEN, "CH%1uOFFSET = %f\n", j+1, rscalibinfo[j-3].offset);
		if (f_puts(line, &file) == 0) {
			PRINTF("Error writing data\r\n");
			f_close(&file);
			return FAIL;
