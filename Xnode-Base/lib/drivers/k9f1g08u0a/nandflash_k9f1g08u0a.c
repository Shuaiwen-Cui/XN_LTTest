...
uint32_t blockNum_remap[NANDFLASH_NUMOF_BLOCK];
bool nandinit = false;
/**
* @}
*/ 
...
* @retval None
*/
void NandFlash_Init(void) // Init only once
{
  uint32_t i,j,count;
	if (nandinit)
		return;
  ...
  */
  ...
  for ( i = 0; i < NANDFLASH_NUMOF_BLOCK; i++ ) {
	...
	for (i=0;i<1011;i++) // use only up to 1011 so that we can compensate for 23 bad blocks
	{
		blockNum_remap[i] = i;
	}
	
	// remap to avoid invalid block
	count = 0;
	for (i=0;i<1011;i++) // use only up to 1011 so that we can compensate for 23 bad blocks
	{
		if (InvalidBlockTable[i] == 1) // invalid block
		{
			for (j=i-count;j<1011;j++)
			{
				blockNum_remap[j] = blockNum_remap[j] + 1;
			}
			count = count+1;
		} 
	}

	for (i=1;i<1011;i++) // use only up to 1011 so that we can compensate for 24 bad blocks
	{
	//	lpc_printf("%d - %d\n\r",i,blockNum_remap[i]);
	}
	nandinit = true;
  return;
}
 ...
 * @retval status
*/
NANDFLASH_STATUS_TypeDef NandFlash_BlockErase(uint32_t blockNum) // NEED MODIFCATION TO RE-MAP THE INVALID BLOCKS
{
  volatile uint8_t *pCLE;
  volatile uint8_t *pALE;
  NANDFLASH_STATUS_TypeDef status;
  ...
  pALE  = K9F1G_ALE;
	if (nandinit){
		blockNum = blockNum_remap[blockNum];
	}
...
*          write up to 2112 bytes of data at a time. // NOTE: Don't try 2112, 2048 is enough as the spare space is kept for marking invalid blocks
...
NandFlash_PageProgram(uint8_t *pBuffer, uint32_t PageAddr, uint32_t ByteAddr, uint32_t NumToWrite) // NEED MODIFCATION TO RE-MAP THE INVALID BLOCKS
{
  volatile uint8_t *pCLE;
  volatile uint8_t *pALE;
  volatile uint8_t *pDATA;
  ...
  pDATA = K9F1G_DATA;
	...
	if (nandinit){
		blockNum = (uint32_t) PageAddr/NANDFLASH_PAGE_PER_BLOCK;
		blockNum = blockNum_remap[blockNum];
		PageAddr = blockNum*NANDFLASH_PAGE_PER_BLOCK + PageAddr%NANDFLASH_PAGE_PER_BLOCK;
	}
...
/**
  ...
  */
NANDFLASH_STATUS_TypeDef 
NandFlash_PageRead(uint8_t *pBuffer, uint32_t PageAddr, uint32_t ByteAddr, uint32_t NumToRead) // NEED MODIFCATION TO RE-MAP THE INVALID BLOCKS
{
  NANDFLASH_STATUS_TypeDef status;
  volatile uint8_t *pCLE;
  volatile uint8_t *pALE;
...
// Source: https://github.com/RT-Thread/realboard-lpc4088/blob/master/software/lpcware_lpc408x/BoardSupport/nandflash_k9f1g08u0a.h
/*********************************************************************//**
...
Bool NandFlash_ValidBlockCheck( void )
{
    ...
    for ( block = 0; block < NANDFLASH_NUMOF_BLOCK; block++ )
    {
			...
			for ( page = 0; page < 2; page++ )
			{
				...
				// Original: NandFlash_PageReadFromAddr(block, page, NANDFLASH_INVALIDBLOCK_CHECK_COLUMM, &data, 1);
				NandFlash_PageRead(&data, block*64+page, NANDFLASH_INVALIDBLOCK_CHECK_COLUMM, 1);
				if(data != 0xFF)
				{
			...
			if (InvalidBlockTable[block] == TRUE){
				PRINTF("Bad Block: %d\r\n", block);
			}
    }
